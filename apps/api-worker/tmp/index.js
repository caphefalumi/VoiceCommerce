...
app.post("/api/auth/register", async (c) => {
  try {
    ...
    if (!email) {
      return c.json({ error: "Email l\xE0 b\u1EAFt bu\u1ED9c" }, 400);
    }
    if (!username) {
      return c.json({ error: "H\u1ECD v\xE0 t\xEAn l\xE0 b\u1EAFt bu\u1ED9c" }, 400);
    }
    if (!password) {
      return c.json({ error: "M\u1EADt kh\u1EA9u l\xE0 b\u1EAFt bu\u1ED9c" }, 400);
    }
...
app.post("/api/orders", async (c) => {
  try {
    ...
    const summary = (Array.isArray(items) ? items : JSON.parse(itemsJson)).map(
      (item, i) => `${i + 1}. ${item.name} x${item.quantity} \u2014 ${(item.price * item.quantity).toLocaleString("vi-VN")} VND`
    ).join("\n");
    return c.json({
      id,
      status: "confirmed",
      total_price,
      confirmation_text: `\u0110\u01A1n h\xE0ng #${id.slice(0, 8).toUpperCase()} \u0111\xE3 \u0111\u01B0\u1EE3c x\xE1c nh\u1EADn!
${summary}
T\u1ED5ng c\u1ED9ng: ${Number(total_price).toLocaleString("vi-VN")} VND. C\u1EA3m \u01A1n b\u1EA1n \u0111\xE3 mua h\xE0ng t\u1EA1i TGDD!`,
...
app.post("/api/tickets", async (c) => {
  try {
    ...
    const categoryLabels = {
      ...
      return_exchange: "\u0110\u1ED5i tr\u1EA3 h\xE0ng",
      warranty: "B\u1EA3o h\xE0nh",
      other: "Kh\xE1c"
    };
    return c.json({
      id,
      ...
      confirmation_text: `Y\xEAu c\u1EA7u h\u1ED7 tr\u1EE3 #${id.slice(0, 8).toUpperCase()} \u2014 ${categoryLabels[category]} \u0111\xE3 \u0111\u01B0\u1EE3c ghi nh\u1EADn. Nh\xE2n vi\xEAn s\u1EBD li\xEAn h\u1EC7 trong 24 gi\u1EDD. Hotline: 1800 2091.`,
